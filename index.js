...
var path          = require('path'),
    ...
    events        = require('judpack-common').events,
    Q             = require('q'),
    CordovaError  = require('judpack-common').CordovaError,
    ConfigParser  = require('judpack-common').ConfigParser,
    fetch         = require('cordova-fetch'),
    url           = require('url'),
    validateIdentifier = require('valid-identifier'),
    CordovaLogger = require('judpack-common').CordovaLogger.get();
...
module.exports = function(dir, optionalId, optionalName, cfg, extEvents) {
    ...
    return Q.fcall(function() {
        ...
        if (!dir) {
            throw new CordovaError('Directory not specified. See `judpack --help`.');
        }
    ...
    .then(function() {
        ...
        events.emit('log', 'Creating a new jud project.');
    ...
    .then(function() {
        ...
        if (isGit || isNPM) {
            ...
            events.emit('log', 'Using judpack-fetch for '+ cfg.lib.www.url);
...
function copyTemplateFiles(templateDir, projectDir, isSubDir) {
    ...
    if (path.basename(templateDir) === 'www') {
        ...
        if (!isSubDir) {
            var excludes = ['RELEASENOTES.md' , '.git', 'NOTICE', 'LICENSE', 'COPYRIGHT', '.npmignore', 'jud-src'];
            templateFiles = templateFiles.filter( function (value) { 
                return excludes.indexOf(value) < 0; 
            }); 
        }
        // create src/index.ju not index.vue
        var isForceWeFile = process.argv.indexOf('--ju')>0;
        ...
        for (var i = 0; i < templateFiles.length; i++) {
            ...
            if (isForceWeFile && templateFiles[i].indexOf('src') >= 0) {
              copyPath = path.resolve(templateDir, 'jud-src/index.ju');
              shell.cp('-R', copyPath, path.join(projectDir, 'src'));
            } else if(isForceWeFile && templateFiles[i] == 'web') {
              // shell.cp('-R', copyPath, projectDir);
              shell.cp('-Rf', path.resolve(templateDir, 'jud-src/web'), projectDir);
            } else {
              shell.cp('-R', copyPath, projectDir);
            }
